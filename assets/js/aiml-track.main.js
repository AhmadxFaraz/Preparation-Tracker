(function () {
  const STORAGE_KEY = 'aiml_ds_track_data';
  ...
  const app = {
    ...
    renderStats: function () {
      ...
      const motivation = document.getElementById('track-motivation');
      if (counts.percent === 0) motivation.textContent = 'Foundations first. Fancy terms later.';
      else if (counts.percent < 25) motivation.textContent = 'You are building the math stack layer by layer.';
      else if (counts.percent < 50) motivation.textContent = 'Now the track is starting to look like real systems thinking.';
      else if (counts.percent < 75) motivation.textContent = 'The models are connecting. Keep going.';
      else if (counts.percent < 100) motivation.textContent = 'Almost there. Stay sharp with the fundamentals.';
      else motivation.textContent = 'This now looks like a serious DS and AI foundation.';

      const strip = document.getElementById('featured-application-strip');
      if (strip) {
        strip.innerHTML = FEATURED_USE_CASES.map(function (item) {
          return '<span class="egt-chip">' + escapeHtml(item) + '</span>';
        }).join('');
      }
    },

    renderSections: function () {
      const target = document.getElementById('track-sections');
      if (!target) return;
          ...
          return (
            '<div class="egt-topic-card' + (topic.completed ? ' is-complete' : '') + '">' +
              '<div class="flex items-start gap-3">' +
                '<input type="checkbox" class="egt-topic-toggle" ' + (topic.completed ? 'checked ' : '') + 'onchange="window.aimlTrack.toggleTopic(' + sectionIndex + ', ' + topicIndex + ')">' +
